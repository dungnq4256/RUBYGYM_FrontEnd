...
function Table({ columns, data, date }) {
    ...
    return (
        <table
            {...getTableProps()}
            className="table table-hover table-bordered table-borderless caption-top tablepopup"
        >
            ...
            <thead className=" table-thread-popup">
            ...
            <tbody className="table-tbody-popup" {...getTableBodyProps()}>
                {rows.map((row, i) => {
                    prepareRow(row);
                    return (
                        <tr {...row.getRowProps()}>
...
function CalendarPopup(props) {
                      ...
                      Cell: (row) => (
                          <input
                              onChange={async (e) => {
                                  ...
                                  if (id) {
                                      const res = await calendarAPI.noteAbsent(
                                          id
                                      );
                                      if (res.data)
                                          if (res.data.status) {
                              ...
                              type="checkbox"
                              defaultChecked={
                                  props.data[row.cell.row.id].absence
                              }
                          />
                      ),
                  },
    ...
    const handleDelete = async (row) => {
        ...
        const response = await calendarAPI.deleteCalendarDaily(
            currentSchedulesId
        );
        // console.log(response);
    };
            ...
            <div className="popupWrapper">
                <div className="popupInner" ref={ref}>
                    ...
                    {props.data && props.data.length > 0 ? (
                        <Table
                            ...
                            data={(function() {
                                ...
                                for (var i = 0; i < props.data.length; i++) {
                                    ...
                                    var start_time_showed =
                                        startHour + ":" + startMinute;

                                    var finishHour = new Date(
                                            calendarList[i].finish_time
                                        )
                                            .getHours()
                                            .toString(),
                                        finishMinute = new Date(
                                            calendarList[i].finish_time
                                        )
                                            .getMinutes()
                                            .toString();
                                    ...
                                    var finish_time_showed =
                                        finishHour + ":" + finishMinute;

                                    calendarList[i].time_showed =
                                        start_time_showed +
                                        " - " +
                                        finish_time_showed;
                                }
                        ...
                        <h2 className="message">Không có dữ liệu</h2>
                    )}
                </div>
